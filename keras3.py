...
def train(data):
    ...
    while len(schools2)<5:
        if type(rank_dic.get(key(max_rank)))!="NoneType":
            schools2.append(rank_dic.get(key(max_rank))[0])
        max_rank=max_rank+1
    max_rank=100-result1
    while len(schools1)<5:
        if type(rank_dic.get(key(max_rank)))!="NoneType":
            schools1.append(rank_dic.get(key(max_rank))[0])
        max_rank=max_rank+1
    while len(schools3)<5:
        if type(rank_dic.get(key(max_rank)))!="NoneType":
            schools3.append(rank_dic.get(key(max_rank))[0])
        max_rank=max_rank+1
    """
